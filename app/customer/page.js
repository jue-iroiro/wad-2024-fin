...
export default function CustomersPage() {
  ...
  const [editId, setEditId] = useState(null); // Define editId as a state variable
  ...
  const startEdit = (customer) => async () => {
    ...
    const formattedDateOfBirth = new Date(customer.dateOfBirth).toISOString().split("T")[0];
  ...
  const createOrUpdateCustomer = async (data) => {
    if (editMode) {
      // Send the PUT request to the correct endpoint with the customer ID
      const response = await fetch(`${APIBASE}/customer/${editId}`, {
        method: "PUT",
        headers: {
  ...
  const cancelEdit = () => {
    reset({ name: "", dateOfBirth: "", memberNumber: "", interests: "" });
    setEditMode(false);
    setEditId(null); // Reset editId when canceling
  };
